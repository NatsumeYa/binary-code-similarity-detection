...
def train_model(args):
    ...
    TEST_FREQ = 1  # 测试模型性能的频率（每隔多少个 epoch 计算 auc）
    SAVE_FREQ = 5  # 保存模型参数的频率（每隔多少个 epoch 保存模型的权重）
    ...
    for i in range(1, MAX_EPOCH + 1):
        ...
        if i % TEST_FREQ == 0:
            ...
            if auc_val > best_auc:
                ...
                # 保存 fpr、tpr 数据（会覆盖同名文件），留着后续画图
                np.savez(ROC_PATH, fpr=fpr_val, tpr=tpr_val)
